...
func main() {
	loggerOptions := []dlog.LoggerOption{
		...
		dlog.LoggerSetPrefix("rail-references: "),
	...
	railReferencesFilename, exists := os.LookupEnv("NAPTAN_CSV_RAIL_REFERENCES_FILENAME")
	if !exists || railReferencesFilename != "" {
		railReferencesFilename = "RailReferences.csv"
	}

	redisHost, exists := os.LookupEnv("RAIL_REFERENCES_REDIS_HOST")
	if !exists || redisHost == "" {
		logger.Fatal("RAIL_REFERENCES_REDIS_HOST not set in environment")
	}

	redisMaxActiveStr, exists := os.LookupEnv("RAIL_REFERENCES_REDIS_MAX_ACTIVE")
	...
	if err != nil {
		logger.Fatal("RAIL_REFERENCES_REDIS_MAX_ACTIVE value is invalid")
	}

	if redisMaxActive < 1 {
		logger.Fatal("RAIL_REFERENCES_REDIS_MAX_ACTIVE value must be greater than 0")
	}
	...
	ln := LocalityNames{
		Filename: railReferencesFilename,
		Logger:   logger,
		NaptanClient: &naptan.Naptan{
			Client: &http.Client{
